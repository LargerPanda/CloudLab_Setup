...
if [ "LIMIT" = $2 ]; then
	...
	do
		...
		ssh -t node-$(($bound - $i)) "sudo cp /users/yushua/CloudLab_Setup/ulimit/limits.conf /etc/security/limits.conf; sudo chmod 664 /etc/pam.d/common-session; sudo echo 'session required pam_limits.so' >> /etc/pam.d/common-session; sudo chmod 644 /etc/pam.d/common-session; ulimit -a; "
	done
fi
